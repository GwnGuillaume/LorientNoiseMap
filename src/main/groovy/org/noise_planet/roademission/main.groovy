...
class Main {
    static void main(String[] args) {
        ...
        String zone_name = "my_zone"    // "zone_PA"

        // Names of the Input tables (.shp) and their attributes
        String in_building_table_name
        String in_area_table_name
        String in_receivers_table_name
        String in_traffic_table_name
        String in_land_use_table_name
        String in_topo_table_name

        // Names of the NoiseModelling tables (.shp) and their attributes
        String nm_building_table_name
        String height_field_name
        String nm_area_table_name
        String nm_receivers_table_name
        String nm_traffic_table_name
        String nm_land_use_table_name
        String nm_topo_table_name

        if (zone_name.equals("zone_PA")) {

            // Names of the Input tables (.shp) and their attributes
            in_building_table_name = "BUILDINGS_ZONE_CAPTEUR"    // "buildings_zone"
            in_area_table_name = "ZONE_CAPTEUR_2154"  // "zone"
            in_receivers_table_name = "RecepteursQuestionnaire"    // "receivers"
            in_traffic_table_name = "ROADS_TRAFFIC_ZONE_CAPTEUR_LITE"  // "roads_traffic_zone_format"
            in_land_use_table_name = "land_use_zone_capteur4"    // "land_use_zone"
            in_topo_table_name = "DEM_LITE"  // "dem_zone"

            // Names of the NoiseModelling tables (.shp) and their attributes
            nm_building_table_name = "buildings"
            height_field_name = "HAUTEUR"    // "height"
            nm_area_table_name = "study_area"
            nm_receivers_table_name = "receivers"
            nm_traffic_table_name = "roads"
            nm_land_use_table_name = "ground_type"
            nm_topo_table_name = "topography"

        } else {
            // Names of the Input tables (.shp) and their attributes
            in_building_table_name =  "buildings_zone"
            in_area_table_name = "zone"
            in_receivers_table_name = "receivers"
            in_traffic_table_name = "roads_traffic_zone_format"
            in_land_use_table_name = "land_use_zone"
            in_topo_table_name = "dem_zone"
...
//            SHPWrite.exportTable(connection, outputPath+"receivers_lden.shp", "receivers_lden".toUpperCase())
